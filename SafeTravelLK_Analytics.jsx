...
// ─── Data (derived from safety_incidents_dataset.csv) ────────────────────────

const YEARLY_DISTRICT = {
  "2010": { Matale: 1 },
  "2011": { Jaffna: 1 },
  "2012": { Monaragala: 1, Colombo: 1 },
  "2013": { Monaragala: 1, Kandy: 1, Colombo: 1 },
  "2014": { "Nuwara Eliya": 1, Colombo: 2 },
  "2015": { Monaragala: 1, "Nuwara Eliya": 1, Matale: 1, Colombo: 1 },
  "2016": { Polonnaruwa: 1, Galle: 1, Matale: 2, Colombo: 3, Monaragala: 2, "Nuwara Eliya": 2, Kandy: 3 },
  "2017": { Jaffna: 1, Colombo: 4, Monaragala: 1, "Nuwara Eliya": 2, Galle: 1, Kandy: 2 },
  "2018": { Colombo: 2, Monaragala: 1, Galle: 4, "Nuwara Eliya": 3, Kandy: 1, Polonnaruwa: 2, Trincomalee: 2, Kegalle: 3 },
  "2019": { "Nuwara Eliya": 4, Colombo: 1, Galle: 4, Kandy: 3, Polonnaruwa: 2, Matale: 2, Kegalle: 3, Anuradhapura: 1 },
  "2020": { Colombo: 4, Kandy: 1, Matale: 1 },
  "2021": { Colombo: 4, Kandy: 3, "Nuwara Eliya": 1 },
  "2022": { Colombo: 4, "Nuwara Eliya": 1, Kandy: 1 },
  "2023": { Matale: 1, "Nuwara Eliya": 1, Galle: 1, Colombo: 1 },
  "2026": { Monaragala: 4, Jaffna: 3, Galle: 5, Colombo: 46, Kandy: 6, Matale: 2, "Nuwara Eliya": 4, Trincomalee: 1 },
};

const YEARLY_SCAM_TYPE = {
  "2013": { "Tuk-Tuk / Transport Scam": 1, "Safety Advisory": 1, "Overcharging": 1 },
  "2014": { "Safety Advisory": 2, "Tuk-Tuk / Transport Scam": 1 },
  "2015": { "Overcharging": 1, "Safety Advisory": 2, "General Safety": 1 },
  "2016": { "Accident / Hazard": 2, "Physical Assault": 1, "General Safety": 2, "Safety Advisory": 6, "Theft / Robbery": 1, "Overcharging": 2 },
  "2017": { "Physical Assault": 1, "Safety Advisory": 6, "Tuk-Tuk / Transport Scam": 3, "General Safety": 1 },
  "2018": { "Safety Advisory": 12, "Tuk-Tuk / Transport Scam": 1, "General Safety": 2, "Overcharging": 3 },
  "2019": { "Gem / Jewellery Scam": 1, "Safety Advisory": 8, "General Safety": 2, "Tuk-Tuk / Transport Scam": 4, "Overcharging": 4, "Harassment / Assault": 1 },
  "2020": { "Safety Advisory": 4, "Tuk-Tuk / Transport Scam": 1, "General Safety": 1 },
  "2021": { "Safety Advisory": 4, "Tuk-Tuk / Transport Scam": 2, "General Safety": 1, "Harassment / Assault": 1 },
  "2022": { "Accident / Hazard": 1, "Gem / Jewellery Scam": 1, "Tuk-Tuk / Transport Scam": 2, "Safety Advisory": 2 },
  "2023": { "Safety Advisory": 2, "Overcharging": 1, "General Safety": 1 },
  "2026": { "General Safety": 17, "Physical Assault": 2, "Accident / Hazard": 1, "Tuk-Tuk / Transport Scam": 13, "Theft / Robbery": 9, "Harassment / Assault": 10, "Safety Advisory": 14, "Overcharging": 1, "Gem / Jewellery Scam": 3 },
};

const CROSS_DISTRICT_PATTERNS = [
  { week: "2026-W32", scam_type: "General Safety Incident", districts: ["Colombo", "Kandy", "Monaragala", "Galle", "Nuwara Eliya"], count: 17 },
  { week: "2026-W32", scam_type: "Safety Advisory", districts: ["Colombo", "Kandy", "Nuwara Eliya", "Monaragala", "Trincomalee"], count: 14 },
  { week: "2026-W32", scam_type: "Tuk-Tuk / Transport Scam", districts: ["Jaffna", "Colombo"], count: 7 },
  { week: "2026-W20", scam_type: "Tuk-Tuk / Transport Scam", districts: ["Matale", "Colombo", "Kandy"], count: 6 },
  { week: "2026-W32", scam_type: "Theft / Robbery", districts: ["Galle", "Colombo"], count: 4 },
  { week: "2026-W32", scam_type: "Gem / Jewellery Scam", districts: ["Colombo", "Kandy"], count: 3 },
  { week: "2026-W20", scam_type: "Theft / Robbery", districts: ["Colombo", "Monaragala"], count: 3 },
  { week: "2019-W01", scam_type: "Tuk-Tuk / Transport Scam", districts: ["Galle", "Nuwara Eliya"], count: 3 },
  { week: "2018-W26", scam_type: "Overcharging", districts: ["Polonnaruwa", "Kandy"], count: 2 },
  { week: "2018-W18", scam_type: "General Safety Incident", districts: ["Polonnaruwa", "Nuwara Eliya"], count: 2 },
];

const SOURCE_WEIGHT_DATA = [
  ...
  { source: "Google News", weight: 0.65, tier: "Aggr", reports: 26, adjusted_risk: 0.36 },
...
const DEMOGRAPHIC_DATA = [
  { label: "Tourists (general)", value: 148, color: "#3B82F6", scam_affinity: ["Tuk-Tuk", "Overcharging", "Gem Scam"] },
  { label: "Travel Vloggers", value: 40, color: "#8B5CF6", scam_affinity: ["Harassment", "Theft", "Physical Assault"] },
];
...
const DISTRICT_COLORS = {
  ...
  Jaffna: "#EC4899", Trincomalee: "#84CC16",
};

// ─── Helpers ──────────────────────────────────────────────────────────────────
function buildTrendData() {
  const years = Object.keys(YEARLY_DISTRICT).sort();
  return years.map(yr => {
    const districtRow = YEARLY_DISTRICT[yr] || {};
    const total = Object.values(districtRow).reduce((a, b) => a + b, 0);
    return { year: yr, total, ...districtRow };
  });
}

function buildScamTrend() {
  const years = Object.keys(YEARLY_SCAM_TYPE).sort();
  return years.map(yr => {
    const row = YEARLY_SCAM_TYPE[yr] || {};
    return { year: yr, ...row };
  });
}

function computeTrendDirection(district) {
  const years = Object.keys(YEARLY_DISTRICT).sort();
  const recent = years.slice(-4);
  const counts = recent.map(y => YEARLY_DISTRICT[y]?.[district] || 0);
...
function TemporalTrendPanel() {
  const [view, setView] = useState("district");
  const trendData = useMemo(() => buildTrendData(), []);
  const scamData = useMemo(() => buildScamTrend(), []);

  const topDistricts = ["Colombo", "Kandy", "Galle", "Nuwara Eliya", "Monaragala"];

  const districtSummary = topDistricts.map(d => ({
    district: d,
    direction: computeTrendDirection(d),
    latest: YEARLY_DISTRICT["2026"]?.[d] || 0,
  }));
  ...
  return (
    <div>
      <SectionHeader label="01 — Temporal Trend Analysis" badge="Actionable">
        Report volume per district over time. Rising trends signal emerging hotspots; falling trends indicate resolved or mis-attributed clusters. 2026 spike reflects August data collection run.
      </SectionHeader>
        ...
        <ResponsiveContainer width="100%" height={300}>
          {view === "district" ? (
            ...
            <LineChart data={scamData} margin={{ top: 8, right: 16, left: -10, bottom: 0 }}>
              ...
              <Tooltip content={<CustomTooltip />} />
              <Legend wrapperStyle={{ fontSize: 11, color: PALETTE.muted }} />
              {Object.keys(SCAM_COLORS).slice(0, 5).map(s => (
...
function DemographicPanel() {
  ...
  const proposed = [
    { id: "solo_female", label: "Solo Female", icon: "👩", risk: 0.71, top_scam: "Harassment / Assault", districts: ["Colombo", "Kandy", "Galle"], evidence: "High co-occurrence with harassment reports; YouTube vlogger accounts skew this demographic" },
    ...
    { id: "vlogger", label: "Travel Vlogger", icon: "🎥", risk: 0.58, top_scam: "Theft / Robbery", districts: ["Colombo", "Jaffna", "Trincomalee"], evidence: "Equipment visibility; 40 reports flagged as Travel Vlogger source — currently unused in scoring" },
  ...
  return (
    <div>
      <SectionHeader label="02 — Demographic Targeting Classifier" badge="Research Value">
        The <code style={{ background: PALETTE.surface, padding: "1px 5px", borderRadius: 3, fontSize: 12 }}>demographic_target</code> field currently contains only "Tourists" and "Travel Vloggers". This panel models the proposed 6-class taxonomy and its research implications.
      </SectionHeader>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
        ...
        <Card>
          <div style={{ color: PALETTE.muted, fontSize: 11, fontFamily: "monospace", marginBottom: 12 }}>CURRENT FIELD DISTRIBUTION</div>
          <ResponsiveContainer width="100%" height={120}>
            <PieChart>
              <Pie data={DEMOGRAPHIC_DATA} dataKey="value" nameKey="label" cx="50%" cy="50%" outerRadius={50} label={({ label, value }) => `${label}: ${value}`}>
                {DEMOGRAPHIC_DATA.map((entry, i) => <Cell key={i} fill={entry.color} />)}
              </Pie>
          ...
          <div style={{ marginTop: 12, padding: 10, background: PALETTE.warning + "11", borderRadius: 8, border: `1px solid ${PALETTE.warning}33` }}>
            ...
            <div style={{ color: PALETTE.muted, fontSize: 11, marginTop: 4 }}>
              Only 2 classes used. Proposed 6-class taxonomy would enable demographic-personalised risk scoring — a publishable methodological contribution.
            </div>
          </div>
        </Card>
      </div>
...
function SourceCredibilityPanel() {
  const [mode, setMode] = useState("unweighted");

  const colomboReports = [
    { source: "TripAdvisor", weight: 0.60, risk_raw: 0.72, count: 30 },
    { source: "YouTube", weight: 0.72, risk_raw: 0.65, count: 15 },
    { source: "Google News", weight: 0.65, risk_raw: 0.80, count: 12 },
    { source: "Sunday Times", weight: 0.85, risk_raw: 0.55, count: 5 },
  ...
  return (
    <div>
      <SectionHeader label="03 — Source Credibility Wiring" badge="Pipeline Fix">
        <code style={{ background: PALETTE.surface, padding: "1px 5px", borderRadius: 3, fontSize: 12 }}>source_weight</code> is stored in the DB but not applied to the risk score formula in <code style={{ background: PALETTE.surface, padding: "1px 5px", borderRadius: 3, fontSize: 12 }}>scoring.py</code>. This shows what changes when you wire it in — modelled on Colombo district.
      </SectionHeader>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
        <Card style={{ textAlign: "center" }}>
          ...
          <div style={{ marginTop: 8, color: PALETTE.muted, fontSize: 11 }}>TripAdvisor (weight 0.60) drives 45% of score</div>
        </Card>
        <Card style={{ textAlign: "center" }}>
      ...
      <Card>
        <div style={{ color: PALETTE.muted, fontSize: 11, fontFamily: "monospace", marginBottom: 12 }}>SOURCE WEIGHT TIER TABLE</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 80px 60px 80px", gap: 0 }}>
          {["Source", "Tier", "Weight", "Reports"].map(h => (
          ...
          {SOURCE_WEIGHT_DATA.map(r => [
            ...
            <div key={r.source + "r"} style={{ color: PALETTE.muted, fontSize: 12, padding: "6px 8px", borderBottom: `1px solid ${PALETTE.border}22` }}>{r.reports || "—"}</div>,
          ])}
        </div>
      </Card>
    </div>
...
function CrossDistrictPanel() {
  ...
  const weekSummary = CROSS_DISTRICT_PATTERNS.reduce((acc, p) => {
  ...
  return (
    <div>
      <SectionHeader label="04 — Cross-District Pattern Linking" badge="Signal Detection">
        Same scam type appearing in multiple districts within the same calendar week is a coordination or seasonality signal — distinct from random noise. Detected via week-window grouping on <code style={{ background: PALETTE.surface, padding: "1px 5px", borderRadius: 3, fontSize: 12 }}>scam_type_normalised × district × ISO week</code>.
      </SectionHeader>
        ...
        <Card>
          {!selected ? (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", color: PALETTE.muted, fontSize: 13 }}>
              ← Select a week to inspect cross-district patterns
            </div>
          ) : (
            <div>
              <div style={{ color: PALETTE.accent, fontFamily: "monospace", fontSize: 12, marginBottom: 16 }}>
                {selected.week} — {selected.total} total reports
              </div>
                    ...
                    <div style={{ marginTop: 10, padding: "8px 12px", background: PALETTE.surface, borderRadius: 8, color: PALETTE.muted, fontSize: 11, lineHeight: 1.5 }}>
                      <strong style={{ color: PALETTE.text }}>Interpretation:</strong> {
                        p.districts.length >= 4
                          ? `Multi-district synchrony suggests a systemic or media-driven reporting event, not an isolated local incident. Worth flagging in the paper as a cascade pattern.`
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ color: PALETTE.accent, fontSize: 12, fontFamily: "monospace", letterSpacing: "0.08em" }}>ADVANCED ANALYTICS</span>
          </div>
        ...
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 11, color: PALETTE.muted, fontFamily: "monospace" }}>n = 188 incidents</div>
          <div style={{ fontSize: 11, color: PALETTE.muted, fontFamily: "monospace" }}>2010 – 2026</div>
        </div>
      </div>
      ...
      <div style={{ padding: "32px", maxWidth: 1200, margin: "0 auto" }}>
        {tab === "trend" && <TemporalTrendPanel />}
        {tab === "demo" && <DemographicPanel />}
        {tab === "source" && <SourceCredibilityPanel />}
        {tab === "cross" && <CrossDistrictPanel />}
      </div>
    </div>
  );
}
