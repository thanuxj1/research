...
HOW TO USE:
  # Auto-download and ingest all known SLTDA PDFs:
      python -m backend.ingest_sltda_pdf --auto

  # Ingest a specific local PDF you already have:
      python -m backend.ingest_sltda_pdf --pdf /path/to/sltda_report_2023.pdf

  # Ingest a PDF from a URL:
      python -m backend.ingest_sltda_pdf --url https://sltda.gov.lk/reports/annual2023.pdf

  # Dry run (print extracted records, do not write to DB):
      python -m backend.ingest_sltda_pdf --auto --dry-run

  # Show all extracted text (debug mode):
      python -m backend.ingest_sltda_pdf --pdf report.pdf --debug

DEPENDENCIES:
  pip install pdfplumber requests beautifulsoup4 lxml

NOTE on academic use:
  These are publicly available government documents. Downloading and processing
  them for academic research is consistent with Sri Lanka's Right to Information
  Act (No. 12 of 2016) and standard academic fair-use principles. Always cite
  the original PDF source in your thesis bibliography.
"""
...
def extract_geo(text: str) -> tuple[str, float, float]:
    t = text.lower()
...
KNOWN_PDF_SOURCES = [
    # ── SLTDA Annual Reports (direct download links — check sltda.gov.lk for updated URLs)
    {
        "slug":   "sltda_official",
    ...
    # ── UK FCDO Sri Lanka Travel Advice (periodically updated — download manually if link is stale)
    {
        "slug":   "fcdo_gov_uk",
...
#  These heuristics extract safety-relevant paragraphs from PDF text.
#  Tuned for SLTDA report structure and government advisory language.
# ══════════════════════════════════════════════════════════════════════════════

# Paragraph-level keywords that indicate safety relevance
...
# Context window: include N sentences before/after a matching sentence
CONTEXT_WINDOW = 2
...
def extract_text_from_pdf(pdf_path: str, debug: bool = False) -> str:
    """Extract all text from a PDF using pdfplumber. Returns joined page text."""
    all_text = []
    try:
...
def extract_tables_from_pdf(pdf_path: str) -> list[dict]:
    """
    Extract tables from PDF pages and convert safety-relevant rows to text records.
    SLTDA statistical reports use tables like:
      | Type of Complaint | 2022 | 2023 |
      | Overcharging      |  145 |  178 |
    These are converted to descriptive text records.
    """
    records = []
    try:
        with pdfplumber.open(pdf_path) as pdf:
            for page_num, page in enumerate(pdf.pages):
                ...
                for table in tables:
                    ...
                    # Check if this looks like a complaint/incident table
                    header_text = " ".join(
                    ...
                    for row in table[1:]:
                        ...
                        # Build a plain-English sentence from the row
                        row_dict = dict(zip(headers, cells))

                        # Skip purely numeric rows or empty rows
                        if not cells[0] or cells[0].isdigit():
                            continue

                        # Generate descriptive text from the row
                        desc_parts = []
...
def split_into_paragraphs(text: str) -> list[str]:
    """Split PDF text into paragraphs by double newline or sentence boundaries."""
    # Split on blank lines first
    paras = re.split(r"\n{2,}", text)
    # Further split very long paragraphs on sentence ends
    result = []
    for para in paras:
...
def extract_safety_paragraphs(full_text: str, debug: bool = False) -> list[dict]:
    """
    Extract paragraphs relevant to tourist safety from PDF full text.
    Uses keyword matching + context window to pull coherent text chunks.
    """
    ...
    for i, para in enumerate(paragraphs):
        ...
        # Build context: include surrounding paragraphs
...
# ══════════════════════════════════════════════════════════════════════════════
#  SCAM TYPE CLASSIFIER (same as news scraper)
# ══════════════════════════════════════════════════════════════════════════════
...
def ingest_records_to_db(
    ...
    """Write extracted records to DB. Returns (inserted, rejected, duped)."""
    ...
    for rec in records:
        ...
        # For SLTDA/Official sources, we relax strict_filter slightly:
        # government reports use formal language that may not match tourist
        # colloquialisms in the filter. We still require SL geo + negative signals
        # but bypass the tourist_score gate for official sources.
        scoring = score_relevance(title, content)
        if source_weight >= 0.90:
            # Tier-0/1 override: pass if geo matches AND there's any negative signal
            passes = scoring["geo_match"] and scoring["negative_score"] > 0
            if not passes and not scoring["geo_match"]:
                rejected += 1
                continue
            # If geo matches but no negative signal, still accept for official sources
            # (statistical records are advisory, not incident-negative)
        ...
        # URL dedup (only when URL is set)
...
def process_pdf(
    ...
    Full pipeline for one PDF:
      1. Extract full text
      2. Extract tables (statistical records)
      3. Extract safety-relevant paragraphs
      4. Ingest to DB
    """
    print(f"\n  Processing: {label}")
    ...
    if not full_text:
        ...
        print("    Consider OCR with: tesseract <pdf> output pdf → then re-run.")
        return 0, 0, 0
...
def main():
    ...
    try:
        if args.auto:
            # ── Auto mode: process all known sources ──────────────────────────
            ...
            for source in KNOWN_PDF_SOURCES:
                if "manual_note" in source:
                    ...
                    print(f"     Once downloaded, run: python ingest_sltda_pdf.py "
                ...
                try:
                    ...
                    time.sleep(2.0)   # polite delay between downloads
